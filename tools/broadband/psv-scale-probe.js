...
function main() {
  ...
  const report = {
    schema: 'quake-sim-psv-scale-diagnosis-v13',
    ...
    verdict: 'fullspace_anchored_layered_schur_validated_locator_infeasible_zero_stable_digits_series_noise_dominated',
    registeredNextStep: 'compensated (double-double) arithmetic across the Schur compliance path (psvPropagator expm products, the above-source m4 chain, the up-leg 2x2 admittance recursion + its inversions, detM): the measured 1-ulp chaos bounds the chain roundoff amplification at cond(M) ~ 1e16 (detM sits at ~1e-16 of |M|^2 in the crest band), so 1e-32-level arithmetic drops the output chaos to ~1e-16 relative. Acceptance gate: the 1-ulp discriminator (chaosDiagnosis) must flip to bit-invariance at 0.70/0.73/0.76/0.80/km — only then does the locator/series agenda re-open on a resolvable field (the current series values are noise-integral dominated at every truncation-safe dh and none is interpretable as signal). Fallback: complex-contour evaluation of the crest band. Production opts.psv stays BLOCKED',
