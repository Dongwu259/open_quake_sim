...
function main() {
  ...
  const layered = {
    ...
    qdSeriesUrm: {
      ...
      measured: '2026-09-12, psv-qd-series.js single-run invocations in parallel (v15 batch; expm4 break test at the optimized n>30 gate). PROVENANCE NOTE (v16): these runs used the legacy double det(Rc) locator windows — v16 moved the locator to the QD detM branch; the v16 detSubtract series (qdDetSubSeriesUrm, QD-locator windows + subtraction) bounds the whole window-treatment effect at <=0.5%, so these literals stand as the plain-series baseline',
      fullTensorUrm: {
    ...
    qdLocatorArm: {
      // v16 (2026-09-12, locator rebuild batch): the registered locator
      // re-run on the resolvable field — frozen literals from
      // tools/broadband/psv-qd-locator.js + the per-candidate fit driver
      // (the full detM scans cost ~57 min each; NOT re-measured live).
      // psvModalPoles gained the params.qdCompliance branch: the SAME dip
      // protocol (surface source zs=1e-4, 2.5e-4/km lattice, prominence
      // 0.5, golden refine, HWHM gamma, halfspace branch exclusion) but
      // the scan object is the Schur detM = det(Y*Q11 - Q21) through the
      // bigfloat chain (at zs~0 the product collapses: detM = det(Y), the
      // surface-admittance pole condition — identical physics to the
      // legacy det(Rc) object).
      method: 'psvModalPoles params.qdCompliance branch (Schur detM landscape, bigfloat chain, opts._qdScanStep coarsens for tests; cache key carries the qd flag)',
      scanTables: {
        f0p5: { scanSecs: 3399, n: 7, candidates: [
          { kInvKm: 0.6003, gammaKm: 0.04154 }, { kInvKm: 0.8562, gammaKm: 0.2211 },
          { kInvKm: 0.8797, gammaKm: 0.06785 }, { kInvKm: 0.9029, gammaKm: 0.05954 },
          { kInvKm: 1.5414, gammaKm: 0.05219 }, { kInvKm: 2.43, gammaKm: 0.08343 },
          { kInvKm: 3.8014, gammaKm: 0.1147 } ] },
        f1p2: { scanSecs: 3469, n: 9, candidates: [
          { kInvKm: 1.6979, gammaKm: 0.08821 }, { kInvKm: 1.722, gammaKm: 0.08821 },
          { kInvKm: 1.742, gammaKm: 0.2079 }, { kInvKm: 1.7626, gammaKm: 0.1833 },
          { kInvKm: 1.783, gammaKm: 0.2005 }, { kInvKm: 1.8032, gammaKm: 0.2229 },
          { kInvKm: 2.2612, gammaKm: 0.3937 }, { kInvKm: 2.9931, gammaKm: 0.1542 },
          { kInvKm: 4.7651, gammaKm: 0.2383 } ] }
      },
      ulpLadderLogdetM: {
        // {k: {v0, ladder}} — v0 = log|detM| at the candidate (1/km label);
        // |v0| > 600 would flag the double-return floor saturation (the
        // f1p2 candidates 1.70-1.80 bottom BELOW it: their v0 here are the
        // dip-adjacent probe points, all unsaturated)
        k0p6003: { v0: 13.83, ladder: '0.00e+0 / 0.00e+0 / 1.37e-13' },
        k0p8562: { v0: 14.8, ladder: '0.00e+0 / 0.00e+0 / 1.10e-13' },
        k0p8797: { v0: 14.68, ladder: '0.00e+0 / 0.00e+0 / -1.26e-12' },
        k0p9029: { v0: 14.35, ladder: '0.00e+0 / 0.00e+0 / -5.08e-13' },
        k1p5414: { v0: 15.2, ladder: '0.00e+0 / 0.00e+0 / 1.37e-13' },
        control0p40: { v0: 15.72, ladder: '0.00e+0 / 5.33e-15 / 2.58e-11' }
      },
      fitTablePerCandidateF0p5: [
        // psvPoleModelFit per candidate INCLUDING failures (rel = 3-channel
        // sqrt(res/norm); the 0.15 gate is psvPoleModelSet's keep gate)
        { cand: 0.6003, g0: 4.15e-2, kp: 0.5801, gamma: 2.44e-3, rel: 0.7412, absA: 2.465e-1, gCand: 5.269e+4 },
        { cand: 0.8562, g0: 2.21e-1, kp: 0.6814, gamma: 4.97e-2, rel: 0.3548, absA: 1.230e+0, gCand: 2.010e+1 },
        { cand: 0.8797, g0: 6.78e-2, kp: 0.5963, gamma: 5.58e-2, rel: 0.2668, absA: 1.619e+0, gCand: 3.354e+0 },
        { cand: 0.9029, g0: 5.95e-2, kp: 0.5734, gamma: 6.59e-2, rel: 0.2490, absA: 5.199e+0, gCand: 2.823e-2 },
        { cand: 1.5414, g0: 5.22e-2, kp: 1.1239, gamma: 8.15e-4, rel: 1.073e-3, absA: 6.592e-23, gCand: 5.087e-33 },
        { cand: 2.43, g0: 8.34e-2, kp: 1.7626, gamma: 1.30e-3, rel: 1.073e-3, absA: 1.301e-47, gCand: 1.091e-63 },
        { cand: 3.8014, g0: 1.15e-1, kp: 2.8838, gamma: 1.79e-3, rel: 1.073e-3, absA: 2.291e-85, gCand: 2.952e-110 }
      ],
      fitTablePerCandidateF1p2: [
        { cand: 1.6979, g0: 8.82e-2, kp: 1.1954, gamma: 1.59e-1, rel: 0.7530, absA: 3.782e+0, gCand: 8.360e+2 },
        { cand: 1.722, g0: 8.82e-2, kp: 1.4673, gamma: 1.38e-3, rel: 0.7604, absA: 1.235e+0, gCand: 1.293e+4 },
        { cand: 1.742, g0: 2.08e-1, kp: 1.4398, gamma: 9.77e-2, rel: 0.6718, absA: 2.216e+0, gCand: 5.694e+3 },
        { cand: 1.7626, g0: 1.83e-1, kp: 1.6020, gamma: 2.66e-2, rel: 0.7363, absA: 4.479e-1, gCand: 4.567e+3 },
        { cand: 1.783, g0: 2.01e-1, kp: 0.3466, gamma: 4.52e-1, rel: 0.7353, absA: 9.133e+0, gCand: 'null (the eps-narrow true-pole null — the QD solve fails AT the candidate)' },
        { cand: 1.8032, g0: 2.23e-1, kp: 1.7258, gamma: 1.50e-2, rel: 0.7141, absA: 2.195e-1, gCand: 1.061e+4 },
        { cand: 2.2612, g0: 3.94e-1, kp: 0.7428, gamma: 1.41e-1, rel: 0.1709, absA: 3.505e+0, gCand: 4.185e-21 },
        { cand: 2.9931, g0: 1.54e-1, kp: 1.7595, gamma: 2.41e-3, rel: 1.073e-3, absA: 1.156e-2, gCand: 2.213e-60 },
        { cand: 4.7651, g0: 2.38e-1, kp: 2.8587, gamma: 3.72e-3, rel: 1.073e-3, absA: 1.186e-59, gCand: 3.098e-126 }
      ],
      integrandProfileF0p5: {
        // |g| max-channel at the QD integrand (fine 0.025/km across the band)
        coarse: '0.30:3.96e4 / 0.40:7.44e3 / 0.50:5.32e4 / 0.60:5.56e4 / 0.70:1.48e4 / 0.80:1.37e4 / 0.85:1.25e2 / 0.90:1.06e-1 / 0.95:7.09e-7 / 1.00:6.79e-11',
        fine: '0.550:8.21e4 / 0.575:2.88e4 / 0.600:5.56e4 / 0.625:2.53e4 / 0.650:2.63e4 / 0.675:5.50e4 / 0.700:1.48e4 / 0.725:5.28e4 / 0.750:2.65e4 / 0.775:4.75e4 / 0.800:1.37e4 / 0.825:8.68e2 / 0.850:1.25e2 / 0.875:1.52e0 / 0.900:1.06e-1 / 0.925:1.26e-4 / 0.950:7.09e-7'
      },
      reading: 'THE LOCATOR REBUILD CLOSES THE POLE AGENDA WITH A MEASUREMENT. (1) The QD detM locator WORKS: the 1-ulp discriminator is exactly 0 at 1-16 ulp and ~1e-13 at 65536 ulps at every probe point (v13 measured the double landscape decorrelating at 0.002/km with bucket minima to -39.5); the noise forest is GONE — f0p5 resolves 17 double-era candidates (8 of them at the 5e-4 gamma floor = noise dips) to 7 real-width dips, f1p2 resolves 45 to 9; the far-band positions MATCH the double detector (2.43/3.80 exact). (2) But the detM dips are NOT integrand poles: dip depths are only 0.5-1.9 log units against a 15.7 background (far-off-axis modes), the buried-source QD integrand is a smooth Bessel-modulated 1e4-9e4 across 0.3-0.8/km with NO pole signatures at any dip, and it collapses exponentially above the halfspace shear branch point omega/vs_half (f0p5: ~0.70/km — 1.4e4 at 0.80 -> 1e2 at 0.85 -> 7e-7 at 0.95; by 1.54/km |g| ~ 1e-33, by 3.8 ~ 1e-110). (3) The single-pole fit class is INAPPLICABLE to this integrand: every supported-band candidate fails the 0.15 gate HONESTLY (rel 0.17-0.76 at f1p2, 0.25-0.74 at f0p5 — the model has no Bessel oscillation to fit), while the dead-band "keeps" (rel identically 1.073e-3, |A| 1e-23..1e-88) are fits to numerical dust. The v12-era "candidates mislocate / fits slide into the 0.70-0.88 thicket" record is thereby RESOLVED: there was nothing to find — the double-era fits were locked onto chain-noise spikes (v13) sitting exactly where the true signal collapses. (4) The only genuine near-axis structure is the f1p2 cluster 1.70-1.80/km (detM bottoming BELOW the 1e-300 double-return floor — essentially exact real-axis zeros, a trapped-mode family just above the f1p2 shear branch point 1.676/km; candidate 1.783 nulls the QD solve at the dip = the eps-narrow true null), but the buried-source integrand is exponentially blind to it (source and receiver couple through evanescent halfspace legs). CS v4 preconditions: P2 (in-band pole resolution) resolves NEGATIVELY — there are no in-band poles to resolve; P3 done (div10); P1 (horizontal absolute anchor) remains the only open gate',
    },
    qdDetSubSeriesUrm: {
      // v16 payoff: the subtraction machinery re-run on the resolved field
      // (frozen literals from tools/broadband/psv-qd-detsub-series.js, six
      // single-run invocations in parallel; 82-128 min per run dominated by
      // the ~57-min QD detM scan inside psvModalPoles; NOT re-measured live)
      measured: '2026-09-12, psv-qd-detsub-series.js single-run invocations in parallel (v16 locator rebuild batch)',
      fullTensorUrm: { dk0p02: 1.242353, dk0p002: 1.242268, dk0p001: 1.236453, dk0p0005: 1.235651 },
      deviatoricUrm: { dk0p02: 0.02227357, dk0p001: 0.0221692 },
      vsPlainQd: { full: '-0.094% / +0.401% / -0.039% / -0.018% at dk 0.02/0.002/0.001/0.0005', dev: '+0.497% / +0.122% at dk 0.02/0.001' },
      verdict: 'detSubtract ON THE QD FIELD IS A MEASURED NO-OP AND THE SERIES STAYS CONVERGED THROUGH IT: every point sits within +-0.50% of the plain QD series (qdSeriesUrm) — subtract+re-add is neutral exactly as the qdLocatorArm reading predicted (no strong poles to subtract; failed fits fall back to the candidate windows). The below-floor spread through the subtraction path is 1.0054 << the 1.03 gate. The detSubtract machinery keeps its double-field role (historical) and its opt-in contract; on the resolved field the plain QD series is the representation'
    },
  ...
  const report = {
    ...
    history: {
      ...
      v16verdict: 'fullspace_anchored_layered_qd_locator_built_band_pole_free_subtract_measured_noop_series_converged',
      v16note: 'the locator rebuild CLOSED the pole agenda by measurement: the QD detM locator (psvModalPoles params.qdCompliance branch) is ulp-exact and resolves the double-era noise forest away (17->7 at f0p5, 45->9 at f1p2, far-band 2.43/3.80 matching), but the dips are far-off-axis (0.5-1.9 log units deep), the buried-source integrand is POLE-FREE (Bessel-modulated smooth 1e4-9e4 over 0.3-0.8/km; exponential collapse above omega/vs_half: 1e-33 by 1.54/km, 1e-110 by 3.8), every supported-band fit fails the 0.15 gate honestly (rel 0.17-0.76), and detSubtract is a measured no-op (+-0.50% of plain, spread 1.0054). The only near-axis structure: the f1p2 1.70-1.80/km trapped-mode cluster with detM below the 1e-300 double-return floor (the integrand exponentially blind to it). CS v4: P2 resolves NEGATIVELY, P3 done, P1 remains. PITFALL RECORDED: the f1p2 ulp ladders first read as noise until the v0 printout showed those dips bottom out below the double-return floor — the ladder was measuring the log() +1e-300 guard jitter, not the chain (|v0| ~ 690 flags saturation); ALSO a 1/km-vs-1/m unit mixup in the driver candidates-file round-trip (silent DC-scale candidates) — units cross the file boundary need an assert',
